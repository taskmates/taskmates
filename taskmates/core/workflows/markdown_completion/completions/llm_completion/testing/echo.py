...
class Echo(GenericFakeChatModel):
    ...
    def _stream(
        ...
        **kwargs,
        ...
        # Start with an empty chunk (role assignment)
        yield ChatGenerationChunk(message=AIMessageChunk(role="assistant", content=""))
        for token in tokens:
            yield ChatGenerationChunk(message=AIMessageChunk(content=token))
        yield ChatGenerationChunk(
            message=AIMessageChunk(content="", finish_reason="stop")
        )
...
async def test_echo_stream_and_generate(tmp_path):
    ...
    result = echo._generate(messages)
    assert result.generations[0].message.content == "Short answer. 1 + 1=?"

    # Test streaming
    chunks = list(echo._stream(messages))
    ...
    async_chunks = []
    async for c in echo._astream(messages):
