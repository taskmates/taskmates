...
from taskmates.defaults.tools.jira_.get_issue_url import get_issue_url
from taskmates.defaults.tools.jira_.get_jira_client import get_jira_client
...
def get_transition_id(jira: JIRA, issue: Issue, status: str) -> Optional[str]:
    """
...
def create_issue(summary: str, description: str, project: str, attachments: Optional[List[str]] = None,
                 status: str = "To Do",
    ...
    if status != "To Do":
        transition_id = get_transition_id(jira, issue, status)
        if transition_id:
            try:
...
def add_comment(issue_key: str, comment: str, attachments: Optional[List[str]] = None) -> str:
    """
...
def get_custom_field_id(field_name: str) -> str:
    """
...
def search_issues(query: str) -> str:
    ...
    """
    jira = get_jira_client()
    goal_field_id = get_custom_field_id('Goal')
    roadmap_field_id = get_custom_field_id('Roadmap')
    category_field_id = get_custom_field_id('Category')
...
def delete_issues(issues: List[str]) -> str:
    """
...
def test_create_issue(tmp_path):
    ...
    result = create_issue(summary, description, project, attachments=[str(attachment)])
...
def test_add_comment(tmp_path):
    ...
    project = "DEMO"
    result = create_issue(summary, description, project)
    issue_key = result.split()[2]
    ...
    result = add_comment(issue_key, comment_text, attachments=[str(attachment)])
...
def test_update_status():
    ...
    result = update_status(issue_key, "Done")
...
def test_search_issues():
    ...
    issue1 = create_issue(summary1, "Test search issue", project, status="Discovery",
                          issue_type='Idea',
                          custom_fields=custom_fields)
    issue2 = create_issue(summary2, "Test search issue", project, status="Done",
                          issue_type='Idea',
                          custom_fields=custom_fields)
    ...
    delete_issues([issue_key1, issue_key2])
...
def test_delete_issues():
    ...
    result1 = create_issue(summary1, "Test issue for deletion", project)
    result2 = create_issue(summary2, "Test issue for deletion", project)
    issue_key1 = result1.split()[2]
    issue_key2 = result2.split()[2]

    result = delete_issues([issue_key1, issue_key2])
...
@pytest.mark.integration
def test_get_custom_field_id():
    goal_field_id = get_custom_field_id('Goal')
    assert goal_field_id is not None

    with pytest.raises(ValueError):
        get_custom_field_id('NonExistentField')


@pytest.mark.integration
def test_read_issue(tmp_path):
    # Create a test issue
    ...
    # Fetch issue details
    issue_details = read_issue(issue_key)
