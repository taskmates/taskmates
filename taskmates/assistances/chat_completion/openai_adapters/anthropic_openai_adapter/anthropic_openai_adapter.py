...
from taskmates.assistances.chat_completion.openai_adapters.anthropic_openai_adapter.convert_and_merge_messages import \
    convert_and_merge_messages
from taskmates.assistances.chat_completion.openai_adapters.anthropic_openai_adapter.convert_anthropic_tool_call_to_openai import \
    convert_anthropic_tool_call_to_openai
from taskmates.assistances.chat_completion.openai_adapters.anthropic_openai_adapter.convert_openai_tools_to_anthropic import \
    ...
    async def create(self,
        ...
        async def inner():
            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    # TODO: Add tracing
                    file_logger.info(f"[anthropic] request_payload.yaml", content=payload)
                    file_logger.info(f"[anthropic] request_payload.json", content=payload)
